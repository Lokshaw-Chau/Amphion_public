...
#     batch_by_size,
# )
from models.tts.speechx.speechx_dataset import (
    SpeechXDataset,
    SpeechXCollator,
    batch_by_size,
)
...
from models.tts.speechx.speechx import SpeechX
import diffusers
from encodec import EncodecModel
from encodec.utils import convert_audio


class SpeechXTrainer(VALLETrainer):
    def __init__(self, args, cfg):
        VALLETrainer.__init__(self, args, cfg)
    ...
    def _build_model(self):
        model = SpeechX(self.cfg.model)

        return model

    def _build_dataset(self):
        return SpeechXDataset, SpeechXCollator
    ...
    def _train_step(self, batch, is_training=True):
        ...
        # extract acoustic features online
        wav = batch["audio"].to(self.device)
        wav = wav.unsqueeze(0)        
        wav = convert_audio(wav, self.cfg.preprocess.sample_rate, self.tokenizer.sample_rate, self.tokenizer.channels)
        wav = wav.permute(1, 0, 2)
        with torch.no_grad():
            audio_features = self.tokenizer.encode(wav)
        audio_features = torch.cat([encoded[0] for encoded in audio_features], dim=-1)
        audio_features_lens = batch["target_len"].to(self.device)
        assert audio_features.ndim == 3
        audio_features = audio_features.permute(0, 2, 1) # [B, T, 8]
        prompt_wav = batch["prompt_wav"].to(self.device)
        prompt_wav = prompt_wav.unsqueeze(0)
        prompt_wav = convert_audio(prompt_wav, self.cfg.preprocess.sample_rate, self.tokenizer.sample_rate, self.tokenizer.channels)
        prompt_wav = prompt_wav.permute(1, 0, 2) # [1, B, T]
        with torch.no_grad():
            prompt_features = self.tokenizer.encode(prompt_wav)
        prompt_features = torch.cat([encoded[0] for encoded in prompt_features], dim=-1) # [B, 8, T]
        prompt_features = prompt_features.permute(0, 2, 1) # [B, T, 8]
        prompt_features_lens = batch["prompt_len"].to(self.device)
        with torch.set_grad_enabled(is_training):
            loss, losses = self.model(
                x=text_tokens,
                x_lens=text_tokens_lens,
                acoustic_prompt=prompt_features,
                acoustic_prompt_lens=prompt_features_lens,
                y=audio_features,
