...
class SpeechX(VALLE):
    def __init__(
        ...
        super().__init__(cfg, decoder_cls, decoder_layer_cls)
    ...
    def forward(
        ...
        x: torch.Tensor,
        x_lens: torch.Tensor,
        acoustic_prompt: Union[torch.Tensor, PromptedFeatures],
        acoustic_prompt_lens: Union[torch.Tensor, PromptedFeatures],
        ...
        **kwargs,
        ...
        Args:
          ...
          acoustic_prompt_lens:   
            A 1-D tensor of shape (N,). It contains the number of tokens in `acoustic_prompt`
          y:
            A 3-D tensor of shape (N, T, 8).
          y_lens:
            A 1-D tensor of shape (N,). It contains the number of tokens in `x`
        ...
        # FIXME: acoustic prompt is PromptedFeatures
        assert not isinstance(acoustic_prompt, PromptedFeatures)

        x_mask = make_pad_mask(x_lens).to(x.device)
        y_mask = make_pad_mask(y_lens).to(y.device)
        y_mask_int = y_mask.type(torch.int64)
        prompt_mask = make_pad_mask(acoustic_prompt_lens).to(y.device)
        prompt_mask_int = prompt_mask.type(torch.int64)
        
        text = x
        codes = y.type(torch.int64) * (1 - y_mask_int.unsqueeze(dim=-1))
        prompt_codes = acoustic_prompt.type(torch.int64) * (1 - prompt_mask_int.unsqueeze(dim=-1))
        ...
        # prompt, _ = self.pad_y_eos(
        #     prompt_codes[..., 0], prompt_mask_int, eos_id=self.audio_token_num
        # )
        prompt = prompt_codes[..., 0]
        self.prompt_mask_int = prompt_mask_int

        metrics = {}
        total_loss = 0.0

        xy_padding_mask = torch.concat([x_mask, prompt_mask ,y_mask], dim=1)
        if self.ar_audio_prepend_bos:
            ar_xy_padding_mask = torch.concat(
                [x_mask, 
                F.pad(y_mask, (1, 0), value=False)], dim=1
            )
        else:
        ...
        if train_stage in [0, 1]:
            ar_loss, ar_metrics = self._forward_ar_decoder(
                text, x_lens.max(), prompt, acoustic_prompt_lens.max(), y, y_lens.max(), targets, x_mask, y_mask, reduction
            )
            total_loss += ar_loss
    ...
    def _forward_ar_decoder(
        self, x, x_len, acoustic_prompt, acoustic_prompt_len , y, y_lens, targets, x_mask, y_mask, reduction
    ):
        ...
        x_attn_mask = F.pad(
            torch.zeros((x_len + acoustic_prompt_len, x_len + acoustic_prompt_len), dtype=torch.bool, device=x.device),
            (0, y_len),
            value=True,
        )
        y_attn_mask = F.pad(
            ...
            (x_len + acoustic_prompt_len, 0),
            value=False,
        )
        xy_attn_mask = torch.concat([x_attn_mask, y_attn_mask], dim=0)

        bsz, src_len = x.shape[0], x_len + y_len + acoustic_prompt_len
        _xy_padding_mask = (
        ...
        prompt_emb = self.ar_audio_embedding(acoustic_prompt)
        prompt_emb = self.ar_audio_prenet(prompt_emb)
        prompt_pos = self.ar_audio_position(prompt_emb)
        
        y_emb = self.ar_audio_embedding(y)
        y_emb = self.ar_audio_prenet(y_emb)
        y_pos = self.ar_audio_position(y_emb)

        xy_pos = torch.concat([x, prompt_pos, y_pos], dim=1)

        xy_dec, _ = self.ar_decoder(
            (xy_pos, None),
            mask=xy_attn_mask,
        )
        logits = self.ar_predict_layer(xy_dec[:, x_len + acoustic_prompt_len:]).permute(0, 2, 1)
