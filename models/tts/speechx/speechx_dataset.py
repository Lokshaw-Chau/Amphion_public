...
class SpeechXDataset(TTSDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.preprocess.use_audio:
            ...
            # single_feature["wav"] = wav.numpy()
            single_feature["target_len"] = single_feature["audio_len"] // 320 + (
                single_feature["audio_len"] % 320 > 0
            )  # 320 is the hop size
            # get prompt from the same speaker
            spk_id = self.utt2spk[utt]
            import random
            random_utt = random.choice(self.spk2utt[spk_id])
            while random_utt == utt:
                random_utt = random.choice(self.spk2utt[spk_id])
            # prompt_path = self.utt2audio_path[random_utt]
            dataset = random_utt.split("_")[0]
            file_name = random_utt.split("#")[-1]
            subset = random_utt.split("_")[1]
            subset = "/".join(subset.split("#")[:-1])
            ...
            # slice wav if it is too long
            if wav.shape[1] > 120000:
                wav = wav[:, : 120000]
            wav = wav.cpu().numpy().squeeze()
            single_feature["prompt_wav"] = wav
            # round to upper integer
            single_feature["prompt_len"] = wav.shape[0] // 320 + (wav.shape[0] % 320 > 0) # 320 is the hop size
...
class SpeechXCollator(TTSCollator):
